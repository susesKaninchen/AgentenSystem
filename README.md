...
- `workflows/research_pipeline.py`: Automatisierter Planner → WebSearchTool (Fallback DuckDuckGo + Seed-Kandidaten) → **ResultFilter-Agent** (dedupliziert Treffer, bevorzugt Kontakt-/About-Seiten) → Evaluator (mit Phase/Region-Presets und Negativfiltern) → **Koordinator- & Supervisor-Agenten** (LLM-Dialoge, Keyword-Hints, Domain-/Organisations-Blacklist) → Directory-Parser → NorthData → Site-Scraper (inkl. Kontakt-/About-Subseiten) → Writer → QA Lauf. Sammelseiten werden automatisch expandiert, ungeeignete Quellen (PDFs, Regierungslisten) sowie Off-Region-Treffer werden vorab gefiltert, und der Site-Scraper legt Kontext-Snapshots der akzeptierten Kandidat*innen ab. Supervisor + Koordinator entscheiden logisch über Dubletten, pflegen die Registry `data/staging/organizations_registry.json` und blocken doppelte Domains. Pro Lauf entstehen mehrere Anschreiben (Standard: 3, per CLI `--letters-per-run` anpassbar). Erzeugt:
  ...
  - Resume-Mode: `--resume-candidates [optional/path]` überspringt jede neue Websuche und nutzt vorhandene Kandidaten-Snapshots (Standard-Datei `data/staging/candidates_selected.json`) – ideal nach API-Limits oder für reine Brief-/QA-Läufe.
...
- Windows-Shortcut: `run_research_pipeline.bat` fragt Phase/Region/Briefe ab, bietet das optionale Seed/Dedupe-Skript an und kann den Resume-Modus (Snapshot-Datei) ohne manuelle CLI-Flags aktivieren.
